...
<head>
    ...
    <title>Lista de Candidatos - Ingenieria Ambiental/title>
...
<nav class="navbar navbar-expand-lg navbar-light bg-dark">
 ...
 <ul class="navbar-nav ml-auto animate_animated animate_bounceInDown" style="padding-right: 50px;">
  ...
  <!-- Main Content -->
  <div class="container">
      <h1 class="text-center" style="margin-top: 20px;">Candidatos - Ingenieria Ambiental</h1>
      <br>
      <br>
      <div class="row">
...
$sql = "SELECT * FROM candidatos_prof WHERE carrera = 'Ingenieria Ambiental'";
...
if ($userStatus == 1) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "            <a href='confirmation_am_pr.php?id=" . htmlspecialchars($row['id']) . "' class='btn btn-primary'>Votar</a>";
            echo "        </div>";
            echo "    </div>";
            echo "</div>";
        }
    } else {
        echo "<p class='text-center'>No se encontraron candidatos de Ingenieria Ambiental.</p>";
    }
}
